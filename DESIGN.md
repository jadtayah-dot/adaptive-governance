...
**One instance.** The globe is mounted once and holds the viewport for the whole page. The well in section five is where it comes to rest at full size, and above the live width it carries no frame: a box drawn around a sphere larger than the box only cuts across it. Below that width the well holds the still and keeps its frame.

**It approaches and it docks.** Asked for on 26 August 2026: the globe should be part of the whole site rather than one block in the middle of it. It is small in the bottom right corner from the hero down, grows to fill the frame across the viewport height before the argument starts, plays the argument unchanged, and shrinks back to the corner across the viewport height after the well has gone by. One value, `presenceAt` in `lib/globe-sequence.ts`, computed from where the argument container sits in the viewport, so a reader scrolling up finds it where they left it.

That is deliberately not the first version, which put the globe in the right margin at a third of the viewport with prose beside it that never referred to it. What made that wallpaper was not the globe being present, it was the globe being inert next to copy that ignored it. Here it is the subject exactly where the copy is about it, and an object with a place of its own everywhere else. The approach happens across the corpus prose, which is the run that says the globe is built from the corpus.

**The size change is a CSS scale, not an altitude or a canvas resize.** Altitude would put the camera about three and a half times further out, where the extrusion stops reading, and resizing the canvas reallocates the drawing buffer every frame of the travel. The canvas is always drawn at viewport size and the docked globe is a downsample of it. `Globe.tsx` measures itself with `offsetWidth`, not `getBoundingClientRect`, because the latter reports the transformed box and the scene would reframe to the docked size and then be scaled again.

**Every section reserves room for it.** `--ag-dock-gutter` in `globals.css`, applied at 1200 and above through `[data-dock-gutter]`. Without it the docked globe floated over the page and covered the names and roles of two partners at 1440. It is a rule in `globals.css` rather than a utility on the sections because Tailwind sorts the arbitrary `min-[1200px]` variant ahead of the named `md` one, so `md:px-10` won and the padding never applied. `tests/homepage.py` measures the real boxes at both widths and fails if anything reaches the card.

**The docked globe takes no pointer input.** Hover and click were built for a sphere filling the frame; at a sixth of that a country is a few pixels, and the tooltip lives inside the scaled layer and would be scaled with it. It becomes the instrument again when it is the subject. `SUBJECT_PRESENCE` is where the line sits.
