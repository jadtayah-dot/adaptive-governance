...
**It is behind the whole page, and it comes forward.** Asked for on 26 August 2026: the globe should be part of the site rather than one block in the middle of it, and it should change as the reader scrolls. It holds the viewport from the hero to the footer as a faint wash under the content, grows to fill the frame as the argument arrives, plays the argument unchanged, and recedes again once the well has gone by.

That is deliberately not the first version, which put the globe in the right margin at a third of the viewport with prose beside it that never referred to it. What made that wallpaper was not the globe being present, it was the globe being inert next to copy that ignored it. Here it is the subject exactly where the copy is about it, and the growth happens across the corpus prose, which is the one run on the page that says the globe is built from the corpus.

It was briefly a small card docked in a corner instead. That needed a gutter down the right of every section to keep out of, and it still covered the names and roles of two partners before the gutter went in. A wash needs no room, so the grids are full width.

**Size and strength are on different curves, and contrast is why.** Size runs across the whole viewport height either side of the argument, which is the part a reader sees as the globe approaching. Strength cannot: for most of that travel there is still copy on screen, and a globe at half opacity behind a paragraph fails 4.5:1 by a wide margin. Strength is therefore measured in pixels of copy rather than in presence. It rises only once the container has pushed the section above it off the top of the screen, over `STRENGTH_RISE_FROM` to `STRENGTH_RISE_TO`, and falls within `STRENGTH_FALL_OVER` pixels of the section below appearing. The fall is much shorter than the rise because the corpus note under the well carries no top padding at all.

**The size change is a CSS scale, not an altitude or a canvas resize.** Altitude would put the camera about three and a half times further out, where the extrusion stops reading, and resizing reallocates the drawing buffer every frame of the travel. The canvas is always drawn at viewport size. `Globe.tsx` measures itself with `offsetWidth`, not `getBoundingClientRect`, because the latter reports the transformed box and the scene would reframe to the scaled size and then be scaled again.
